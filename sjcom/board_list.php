...
<body>
    ...
    <div class="section_board">
        ...
        <div class="board_top">
            <p>&diams; 총 <?php
                $sql="select * from sjcom_board";
                $result = mysqli_query($conn, $sql);
                echo mysqli_num_rows($result);
            ?>개의 게시물이 있습니다.</p>

            <form action="" class="search_form">
                <select name="find_by" id="find_by">
            ...
            <table>
                ...
                <tbody>
                    ...
                    $start = ($page - 1) * $list_num; //페이지 시작 번호
                        $sql = "select * from sjcom_board order by num desc limit $start, $list_num";
                        $result = mysqli_query($conn, $sql);
                        while ($row = mysqli_fetch_array($result)) {
                    ?>
                    <tr>
                        <td><?php echo $row['num']; ?></td>
                        <td><a href="board_view.php?num=<?php echo $row['num'];?>"><?php echo $row['subject']; ?></a></td>
                    ...
                    <tr>
                        <td colspan="5">
                            <?php //이전 페이지
                                if ($page <= 1) {
                            ?>
                                    <a href="board_list.php?page=1" class="prev">&blacktriangleleft;</a>
                            <?php
                                } else {
                            ?>
                                    <a href="board_list.php?page=<?php echo ($page - 1);?>" class="prev">&blacktriangleleft;</a>
                            <?php }?>
                            ...
                            ?>
                                    ...
                                    <?php } else {?>
                                        <a href="board_list.php?page=<?php echo $print_page;?>" <?php if ($page != $print_page) echo 'class="page"';?>>[<?php echo $print_page;?>]</a>
                                    <?php } ?>
                            <?php }?>
                            ...
                            <?php //다음 페이지
                                if($page >= $total_page) {
                            ?>
                                    <a href="board_list.php?page=<?php echo $total_page;?>" class="next">&blacktriangleright;</a>
                            <?php } else {?>
                                    <a href="board_list.php?page=<?php echo ($page + 1); ?>" class="next">&blacktriangleright;</a>
                            <?php
                                }
                            ?>
                        </td>
